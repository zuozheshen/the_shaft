...
- 乘客未登舱时，右操作台可直接前往楼层数据库中存在的任意楼层；楼层不存在时会显示无法前往。乘客登舱后，前往楼层才需要先完成地址验证。
- 单纯抵达任何楼层都不会触发接乘、乘客进入/离开、`destination_楼层号` 或案例结束。
- 接乘前到达 `612` 后，会进入“已抵达接乘点”并显示门外摄像头证据；玩家仍须完成门外确认并开门，乘客才会进入舱内。此时不触发 `destination_612`。
- 若门外确认完成但乘客尚未登舱，玩家可以离开 `612`；流程会回到等待接乘，门外对话选项与旧乘客台词会被清除。
...
1. 运行主场景。
2. 在右操作台输入并前往 `612`；确认停靠后舱门保持关闭，主操作台任务改为使用门外摄像头确认等待乘客。
3. 切换门外摄像头，确认出现 `612` 层的门外证据；此时乘客尚未进入舱内。
4. 主操作台开麦，确认出现 `pickup_start` 里的门外对话。
5. 选择门外确认选项后点击开门，确认乘客进入舱内。
6. 点击关门，确认进入舱内问答。
