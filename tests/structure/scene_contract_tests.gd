...
func run() -> Array[String]:
	...
	config.load("res://project.godot")
	config.erase_section_key("autoload", Validator.GODOT_AI_AUTOLOAD_NAME)
	validator = Validator.new()
	validator.validate_configuration(config)
	_expect("Godot AI helper autoload required", str(validator.errors).contains(Validator.GODOT_AI_AUTOLOAD_NAME))
	config.load("res://project.godot")
	var enabled_plugins: PackedStringArray = config.get_value("editor_plugins", "enabled")
	enabled_plugins.remove_at(enabled_plugins.find(Validator.GODOT_AI_PLUGIN))
	config.set_value("editor_plugins", "enabled", enabled_plugins)
	validator = Validator.new()
	validator.validate_configuration(config)
	_expect("Godot AI editor plugin required", str(validator.errors).contains(Validator.GODOT_AI_PLUGIN))
	config.load("res://project.godot")
